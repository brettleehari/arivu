...
| Adaptive foreground | Android | `res/drawable/ic_launcher_foreground.xml` | All geometry inside the 66dp safe circle (r 33 around 54,54) — survives circle, squircle and teardrop masks |
| Adaptive background | Android | `res/drawable/ic_launcher_background.xml` | Brand green, faint diagonal gradient #2A6E5A → #1A4F40 |
| Themed icon (Android 13+) | Android | `res/drawable/ic_launcher_monochrome.xml` | Single silhouette; lines and spark cut out (evenOdd) |
...
| App icon | iOS | asset catalogue, 1024×1024 | One opaque square, no alpha, no rounding of our own — the system masks it. Rendered from the same SVG as the Play icon, full-bleed, with the mark inside the same safe circle so the corner radius never clips it |
...
| Launch screen | both | `values-v31/themes.xml` / `UILaunchScreen` | Mark on brand green in light and dark on both: the first screen looks like the icon that was tapped |
...
| Store icon | both | `leaves/design/store-icon.svg` → `store-icon-512.png` | 512×512 for Play; the same SVG at 1024×1024 for the App Store. Re-render both after any geometry change |

Rules: the Android vectors, the iOS asset and the SVG share path data verbatim — change one, change all.
No text in the icon (no wordmark to translate).
